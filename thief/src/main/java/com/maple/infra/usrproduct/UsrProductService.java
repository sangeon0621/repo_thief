...
public class UsrProductService {
	...
	public int insertBuyList(UsrProductDto usrProductDto) {
	 ...
	 // 카카오페이 결제창 연결
    public UsrProductDto payReady(String name, int totalPrice) {
        ...
        ResponseEntity<UsrProductDto> responseEntity = template.postForEntity(url, requestEntity, UsrProductDto.class);
    ...
    public UsrProductDto payApprove(String tid, String pgToken) {
        Map<String, String> parameters = new HashMap<>();
        parameters.put("cid", "TC0ONETIME");              // 가맹점 코드(테스트용)
        ...
        UsrProductDto approveResponse = template.postForObject(url, requestEntity, UsrProductDto.class);
